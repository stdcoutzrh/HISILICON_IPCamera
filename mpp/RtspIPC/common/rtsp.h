...
#include "list.h"
#if !defined(WIN32)
	#define __PACKED__        __attribute__ ((__packed__))	//结构体内元素字节对齐
#else
	#define __PACKED__ 
#endif
...
typedef struct 
{
	...
	int startblock;//代表开始文件块叿
	int endblock;//代表结束文件块号
	int stampnum;//代表时间戳数釿
}IDXFILEBLOCK_INFO;//.IDX文件段信恿

typedef struct 
{
	int blockindex;//代表所在文件块叿
	int pos;//代表该时间戳在文件块的位罿
	_int64 time;//代表时间戳时间戳的时间点
}IDXSTAMP_INFO;//.IDX文件的时间戳信息

typedef struct 
{
	char filename[150];//代表所在文件块叿
	int pos;//代表该时间戳在文件块的位罿
	_int64 time;//代表时间戳时间戳的时间点
