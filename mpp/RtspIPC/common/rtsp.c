...
static char const* dateHeader()
{
	...
	time_t tt = time(NULL);
	strftime(buf, sizeof buf, "Date: %a, %b %d %Y %H:%M:%S GMT\r\n", gmtime(&tt));//gmtime是把日期和时间转换为格林威治(GMT)时间的函数
#endif
...
int ParseRequestString(char const* reqStr,unsigned reqStrSize,
		       ...
		       char* resultCSeq,unsigned resultCSeqMaxSize) 
{
	...
	{
		if ((reqStr[j] == 'r' || reqStr[j] == 'R')&& (reqStr[j+1] == 't' || reqStr[j+1] == 'T')
				&& (reqStr[j+2] == 's' || reqStr[j+2] == 'S')&& (reqStr[j+3] == 'p' || reqStr[j+3] == 'P')
...
void ParseTransportHeader(char const* buf,StreamingMode* streamingMode,
